...
Refactor the existing `spaced-repetition-deck-generation` project into a Model Context Protocol (MCP) server. The server is the single distribution unit: it ships filesystem primitives, Mochi API primitives, sync primitives, image-processing primitives, subagent prompts, and workflow prompts. Any MCP-capable client (Claude Code, Claude Desktop, Cursor, Goose, Zed) can install the server and immediately gain a full deck-management workflow.
...
Default `DECKGEN_DECKS_ROOT` is `~/.local/share/deckgen-mcp/decks/`, not `./decks/`. Casual users never see the filesystem; workflows phrase actions in terms of Mochi decks ("added 50 cards to *Flags* on Mochi"), not local folders. Power users override `DECKGEN_DECKS_ROOT=./decks` to hand-edit cards in their editor — that flow remains fully supported.
...
Optional extras: `pip install 'deckgen-mcp[svg]'` enables SVG → PNG conversion via `cairosvg` (for Wikipedia flag SVGs, etc.).
...
| `DECKGEN_DECKS_ROOT` | Optional | `~/.local/share/deckgen-mcp/decks/` | Where local decks live. Override to `./decks` for hand-editing |
...
- SVG conversion failure (cairosvg not installed) → warn, save the raw SVG, suggest `pip install 'deckgen-mcp[svg]'`.
...
- `test_prompt_compression.py` — NEW. Asserts every file in `src/deckgen_mcp/prompts/agents/` has ≤15 non-blank lines and every file in `src/deckgen_mcp/prompts/workflows/` has ≤30 non-blank lines. Fails the build if a prompt grows past the cap.
...
- Code coverage on `src/deckgen_mcp/local/`, `mochi/`, `sync/`, `tools/` ≥ 90%.
- `ruff check .` — zero warnings.
- `ruff format --check .` — formatted.
- `mypy src/deckgen_mcp/` — clean under strict mode (`--strict`).
